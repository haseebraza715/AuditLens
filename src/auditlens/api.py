...
def _layer1_only_final_report(
    layer1_report: dict[str, Any],
    summary: dict[str, Any],
) -> dict[str, Any]:
    """Layer-2-shaped payload for PDF export when Layer 2 was not run."""
    issues = []
    for item in list(layer1_report.get("issues", []) or []):
        issues.append(
            {
                "statistical_issue": item,
                "interpretation": {
                    "issue_id": str(item.get("issue_id", "unknown")),
                    "why_harmful": "Layer 2 interpretation was not run; review this statistical finding manually.",
                    "at_risk_groups": [],
                    "likely_model_impact": "Potential fairness or data-quality risk requires manual review.",
    ...
    return {
        ...
        "summary": (
            f"Deterministic statistical audit completed: {summary.get('total_issues', 0)} issue(s) "
            f"({summary.get('high_severity', 0)} high, {summary.get('medium_severity', 0)} medium, "
            f"{summary.get('low_severity', 0)} low)."
        ),
        "task_context": {
            "task_type": "unknown",
        ...
        "issues": issues,
        "disclaimer": (
            "Generated by AuditLens Layer 1 statistical checks only; no LLM interpretation was used. "
            "Human review is recommended before deployment decisions."
        ),
        "reproducibility": {
            "generated_at_utc": "",
            "request_id": "",
            ...
            "layer2_model": "none",
            "severity_thresholds": layer1_report.get("severity_thresholds", {}),
        },
    }
...
class AuditLensReport:
    ...
    def to_pdf(self, path: str, *, generated_at_utc: datetime | None = None) -> None:
        ...
        Works for Layer 1-only reports and completed Layer 2 reports; requires
        the optional `pdf` extra (ReportLab + matplotlib).
        """
        if self._interpretation and self._interpretation.get("status") == "complete":
            final = self._interpretation.get("final_report") or {}
        else:
            final = _layer1_only_final_report(self._layer1_report, self.summary)
